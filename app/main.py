...
from app.database import (
    ...
    AnalyticsSnapshot, ThreatEvent, LoginAttempt,
)
...
async def analytics_timeseries(
    ...
    return {
        "labels": [r.timestamp.strftime("%Y-%m-%d %H:%M") for r in rows],
        "requests_total": [r.requests_total for r in rows],
        "requests_cached": [r.requests_cached for r in rows],
        "requests_uncached": [r.requests_uncached for r in rows],
        "bandwidth_total_mb": [round(r.bandwidth_total_bytes / 1_000_000, 2) for r in rows],
        "bandwidth_cached_mb": [round(r.bandwidth_cached_bytes / 1_000_000, 2) for r in rows],
        "threats_total": [r.threats_total for r in rows],
...
async def analytics_summary(
    ...
    total_requests = sum(r.requests_total for r in rows)
    total_cached = sum(r.requests_cached for r in rows)
    total_bandwidth = sum(r.bandwidth_total_bytes for r in rows)
    total_threats = sum(r.threats_total for r in rows)
    ...
    return {
        ...
        "last_updated": rows[-1].timestamp.isoformat() if rows else None,
    }
